...
public class EventoDao {
    ...
    public ResultSet busca(){
        ...
        try {
            ...
            psmt = conn.prepareStatement("SELECT * FROM evento");
    ...
    public ResultSet buscar(int pk){
        ...
        ResultSet rset = null;
        try {
            psmt = conn.prepareStatement("Select * from evento where id = ? ");
            psmt.setInt(1,pk);
            rset = psmt.executeQuery();
            return rset;
        }catch (SQLException sqle){
    ...
    public int inserirEvento(Evento evento){
        conexao.conectar();
        try {
            psmt = conn.prepareStatement("INSERT INTO evento(faixa_etaria,descricao,categoria,capacidade,horario,data_inicio,data_termino,preco_pessoa,id_atracao,id_tour_virtual) VALUES(?,?,?,?,?,?,?,?,?,?) ");
    ...
    public int alterarFaixaEtaria(Evento evento, int id){
        ...
        try {
            ...
            if (busca.next()){
                psmt = conn.prepareStatement("UPDATE evento SET faixa_etaria = ? WHERE id = ? ");
                psmt.setString(1, evento.getFaixa_etaria());
                psmt.setInt(2, id);
                psmt.execute();
                return 1;
    ...
    public int alterarDescricao(Evento evento, int id){
        ...
        try {
            ResultSet busca = buscar(id);
            if (busca.next()){
                psmt = conn.prepareStatement("UPDATE evento SET descricao = ? WHERE id = ? ");
    ...
    public int alterarCategoria(Evento evento, int id){
        ...
        try {
            ResultSet busca = buscar(id);
            if (busca.next()){
                psmt = conn.prepareStatement("UPDATE evento SET categoria = ? WHERE id = ? ");
    ...
    public int alterarCapacidade(Evento evento, int id){
        ...
        try {
            ResultSet busca = buscar(id);
            if (busca.next()){
                psmt = conn.prepareStatement("UPDATE evento SET capacidade = ? WHERE id = ? ");
    ...
    public int alterarHorario(Evento evento, int id){
        ...
        try {
            ResultSet busca = buscar(id);
            if (busca.next()){
                psmt = conn.prepareStatement("UPDATE evento SET horario = ? WHERE id = ? ");
    ...
    public int alterarDataInicio(Evento evento, int id){
        ...
        try {
            ResultSet busca = buscar(id);
            if (busca.next()){
                psmt = conn.prepareStatement("UPDATE evento SET data_inicio = ? WHERE id = ? ");
    ...
    public int alterarDataTermino(Evento evento, int id){
        ...
        try {
            ResultSet busca = buscar(id);
            if (busca.next()){
                psmt = conn.prepareStatement("UPDATE evento SET data_termino = ? WHERE id = ? ");
    ...
    public int alterarPrecoPessoa(Evento evento, int id){
        ...
        try {
            ResultSet busca = buscar(id);
            if (busca.next()){
                psmt = conn.prepareStatement("UPDATE evento SET preco_pessoa = ? WHERE id = ? ");
    ...
    public int deletar(int id){
        ...
        try {
            ResultSet busca = buscar(id);
            if (busca.next()){
                psmt = conn.prepareStatement("DELETE FROM evento WHERE id = ? ");
                psmt.setInt(1, id);
                psmt.execute();
                return 1;
            }
